...
<div class="row">
    ...
    <div class="alert alert-danger" role="alert">
        <i class="dripicons-wrong me-2"></i> <strong>{{\Session::get('ImportedRec') }}</strong> has been imported out of <strong>{{ \Session::get('totalRec')}}</strong>
    </div>
    @endif
</div>
<div class="card">
               ...
               <tbody>
                ...
                @foreach ($employees as $data)
                 <tr>
                   ...
                   <td>{{$data->dob}}</td>
                   <td>{{$data->position}}</td>
                   <td class="text text-capitalize">{{$data->type_of_employee}} @if($data->type_of_employee=="hourly") <span style="font-size: 12px !important" class="badge badge-dark-lighten">  ( hrs {{$data->number_of_hours}} rate {{$data->amount_per_hour}}) </span>  @endif </td>
 ...
 @section('scripts')
    ...
    @if(\Session::has('empSuccessUpdate'))
    <script>
   ...
   @if(\Session::has('empSuccessDelete'))
   <script>
  ...
  @if(\Session::has('empSuccessImport'))
  <script>
